...
func (m *Model) renderFooter() string {
	help := "Grid: стрелки/h j k l, Enter=открыть, w=смотреть, m=плеер, f=избранное, 0..5=список, b=закладки, g=главная, i=вход, u=регистрация, o=выход, Tab/[ ]=категории, n/p=страницы, r=reload, q=выход"
	if m.voiceoverPickerOpen {
		help = "Voiceover: ↑/↓ или j/k выбрать, Enter подтвердить, 1..9 быстрый выбор, Esc отмена"
	} else if m.auth != nil {
	...
	if m.width < 100 && m.auth == nil && !m.voiceoverPickerOpen {
		if m.mode == modeDetail {
			...
			help = "Grid: move стрелки/hjkl | Enter | w | m | Tab | n/p | f | 0..5 | b/g | i/u/o | q"
		}
	}
...
func (m *Model) beginVoiceoverSelectionCmd() tea.Cmd {
	...
	if releaseID <= 0 {
		m.statusText = "Сначала выберите релиз для просмотра"
		return nil
	}
...
func (m *Model) watchByReleaseCmd(releaseID int, voiceoverID int) tea.Cmd {
	...
	m.errText = ""
	m.statusText = "Идет подготовка видео и запуск плеера..."
	return func() tea.Msg {
